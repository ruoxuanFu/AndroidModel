...
class DateUtils private constructor() {
    companion object {
        ...
        val instance: DateUtils by lazy(mode = LazyThreadSafetyMode.SYNCHRONIZED) {
            DateUtils()
        }
    ...
    val FORMAT_NO_SECOND = "yyyy-MM-dd HH:mm"
    val FORMAT_UNDERLINED = "yyyy-MM-dd_HH-mm-ss"
    val FORMAT_NO_MODIFICATION = "yyyyMMddHHmmss"
    val FORMAT_CN = "yyyy年MM月dd日 HH:mm:ss"
    val FORMAT_CN_NO_SECONDS = "yyyy年MM月dd日 HH:mm"
    val FORMAT_T = "yyyy-MM-dd'T'HH:mm:ss"

    @IntDef(MILLISECOND, SECOND, MINUTE, HOUR, DAY)
    @Retention(AnnotationRetention.RUNTIME)
    annotation class Unit

    private val mSdfThreadLocal = ThreadLocal<SimpleDateFormat>()

    private fun getSimpleDateFormat(format: String = FORMAT_DEFAULT): SimpleDateFormat {
        var simpleDateFormat: SimpleDateFormat? = mSdfThreadLocal.get()
        if (simpleDateFormat == null || !format.isSame(FORMAT_DEFAULT)) {
            mSdfThreadLocal.remove()
            simpleDateFormat = SimpleDateFormat(format, Locale.getDefault())
            mSdfThreadLocal.set(simpleDateFormat)
        }
    ...
    fun getFitTimeSpan(
        ...
        val delta = string2Millis(time1, getSimpleDateFormat()) - string2Millis(
            time2,
            getSimpleDateFormat()
        )
     ...
     * @return the current formatted time string
     */
    fun getNowString(@NonNull format: DateFormat): String =
    ...
    fun getNetTime(readTime: Int, connectTime: Int): Long {
        return try {
            val uc = URL("http://www.baidu.com").openConnection()
